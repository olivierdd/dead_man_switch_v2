...
class TestTokenCleanupService:
    ...
    async def test_cleanup_expired_tokens_with_expired(self, cleanup_service, mock_db, sample_tokens):
        ...
        expired_tokens = [token for token in sample_tokens if token.expires_at < datetime.utcnow()]
    ...
    async def test_cleanup_expired_tokens_with_specific_types(self, cleanup_service, mock_db, sample_tokens):
        ...
        email_tokens = [token for token in sample_tokens if token.token_type == TokenType.EMAIL_VERIFICATION]
        expired_email_tokens = [token for token in email_tokens if token.expires_at < datetime.utcnow()]
    ...
    async def test_get_cleanup_statistics(self, cleanup_service, mock_db):
        ...
        # Mock database queries
        mock_db.exec.return_value.first.side_effect = [100, 25, 50, {"email_verification": 60, "password_reset": 40}, 10]
