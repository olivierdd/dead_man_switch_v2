...
from sqlmodel import SQLModel, create_engine, Session, Field, Index
...
from .message import (
    Message, MessageShare, Recipient, DissolutionPlan, MessageAttachment,
    MessageAuditLog, MessageCreate, MessageUpdate, MessageShareCreate,
    RecipientCreate, MessageResponse, MessageCheckIn, MessageSearch
)

# Database configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgresql://user:password@localhost/secret_safe")
SUPABASE_URL = os.getenv("SUPABASE_URL", "")
SUPABASE_ENABLE_RLS = os.getenv(
    "SUPABASE_ENABLE_RLS", "true").lower() == "true"
...
class DatabaseIndexes:
    ...
    USER_STATUS_INDEX = Index(
        "idx_user_status", "user.is_active", "user.is_suspended")
    USER_SUBSCRIPTION_INDEX = Index(
        "idx_user_subscription", "user.subscription_tier", "user.subscription_expires_at")
    ...
    MESSAGESHARE_USER_INDEX = Index(
        "idx_messageshare_user", "messageshare.shared_with_user_id", "messageshare.expires_at")
    ...
    RECIPIENT_DELIVERY_INDEX = Index(
        "idx_recipient_delivery", "recipient.delivery_method", "recipient.delivery_status")

    # Audit log indexes
    AUDIT_USER_ACTION_INDEX = Index(
        "idx_audit_user_action", "userauditlog.user_id", "userauditlog.action", "userauditlog.created_at")
    AUDIT_MESSAGE_ACTION_INDEX = Index(
        "idx_audit_message_action", "messageauditlog.message_id", "messageauditlog.action", "messageauditlog.created_at")
    ...
    ATTACHMENT_MESSAGE_INDEX = Index(
        "idx_attachment_message", "messageattachment.message_id", "messageattachment.file_hash")
    ATTACHMENT_TYPE_INDEX = Index(
        "idx_attachment_type", "messageattachment.mime_type", "messageattachment.file_size")
