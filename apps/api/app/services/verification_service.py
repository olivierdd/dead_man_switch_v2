...
from ..models.verification import (TokenManager, TokenType,
                                   TokenValidationRequest,
                                   TokenValidationResponse, VerificationToken,
...
class VerificationService:
        ...
        try:
            ...
            if not verification_token:
                ...
                return TokenValidationResponse(
                    valid=False, message="Invalid verification token"
                )
            ...
            if verification_token.expires_at < datetime.utcnow():
                ...
                return TokenValidationResponse(
                    valid=False, message="Verification token has expired"
                )
            ...
            if verification_token.used_at:
                ...
                return TokenValidationResponse(
                    valid=False, message="Verification token has already been used"
                )
    ...
    async def send_verification_email(
        self, user: User, token: str, base_url: str, expiry_hours: int = 24
    ) -> bool:
    ...
    async def send_welcome_email(self, user: User, base_url: str) -> bool:
    ...
    async def resend_verification_email(
        self, user_id: UUID, base_url: str, expiry_hours: int = 24
    ) -> bool:
        ...
        try:
            ...
            if not user:
                logger.error(f"User {user_id} not found for resending verification")
                return False
            ...
            success = await self.send_verification_email(
                user=user, token=token, base_url=base_url, expiry_hours=expiry_hours
            )
