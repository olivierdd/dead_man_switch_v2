        ...
        with open(env_file, 'r') as f:
            for line in f:
                ...
                if line and not line.startswith('#') and '=' in line:
                    ...
                    print(f"Loaded: {key} = {value[:20]}..." if len(
                        value) > 20 else f"Loaded: {key} = {value}")
...
def list_tables(supabase: Client):
    ...
    for table in common_tables:
        try:
            ...
            print(
                f"   Columns: {list(result.data[0].keys()) if result.data else 'No data'}")
            print(f"   Row count: {len(result.data)}")

            # Show sample data
            if result.data:
                print(
                    f"   Sample data: {json.dumps(result.data[0], indent=2, default=str)}")
...
def test_auth_tables(supabase: Client):
    ...
    try:
        ...
        print(
            f"   Sample: {json.dumps(result.data[0] if result.data else {}, indent=2, default=str)}")
    except Exception as e:
    ...
    try:
        ...
        if result.user:
            ...
            if user_result.data:
                ...
                print(
                    f"   Profile data: {json.dumps(user_result.data[0], indent=2, default=str)}")
            else:
